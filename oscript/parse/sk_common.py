...
class IssueAST(object):
    ...
    *** When you update sk_parser, you need to update this class
    ...
    def issue_exec(self, ast):
        p = [self.issue(a) for a in ast.items]
        if p[3] == 'None':
    ...
    def issue_param_list(self, ast):
        params = [self.issue(a) for a in ast.items]
        return " ".join(params)

    issue_kwd_params = issue_param_list
    ...
    def issue_term(self, ast):
        items = [self.issue(a) for a in ast.items]
        return ' '.join(items)
    ...
    def issue_block(self, ast):
        stmts = [self.issue(a) for a in ast.items]
        return "{\n%s\n}" % ('\n'.join(stmts))
    ...
    def issue_while(self, ast):
        pred, body = [self.issue(a) for a in ast.items]
        return "WHILE %s\n%s" % (pred, body)
    ...
    def issue_calc(self, ast):
        exp = self.issue(ast.items[0])
        return "CALC %s" % (exp)

    def issue_catch(self, ast):
        var, body = [self.issue(a) for a in ast.items]
        return "CATCH %s\n%s" % (var, body)

    def issue_return(self, ast):
        exp = self.issue(ast.items[0])
        return "RETURN %s" % (exp)

    def issue_skeleton(self, ast):
        (params, body) = [self.issue(x) for x in ast.items]
        # ??
        return body

    def issue_command_section(self, ast):
        (pre, body, post) = [self.issue(x) for x in ast.items]
        return ':START\n%s\n:MAIN_START\n%s\n:MAIN_END\n%s\n:END\n' % (
            pre, body, post)

    def issue_star_sub(self, ast):
        (cmd, params) = [self.issue(x) for x in ast.items]
    ...
    # NOT handled yet: if_list, cond, star_if, star_set, star_for, idlist, asn, let
